    ...
    <div class="container-custom">
      <div class="max-w-3xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-8">
          <h1 class="text-3xl font-bold mb-8">Edit Update</h1>
          
          <form v-if="updateData" @submit.prevent="saveUpdate" class="space-y-6">
            <div>
            ...
            <div>
              ...
              <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-neutral-300 border-dashed rounded-md">
                <div class="space-y-1 text-center w-full">
                  ...
                  <div v-if="existingMediaPreviews.length > 0" class="mb-4">
                    <h4 class="text-sm font-medium text-neutral-700 mb-2">Current Media</h4>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                      <div 
                        v-for="(preview, index) in existingMediaPreviews" 
                        :key="'existing-' + index" 
                      ...
                      >
                        ...
                        <button
                          type="button"
                          @click="removeExistingMediaItem(index)"
                  ...
                  <div v-if="mediaPreviews.length > 0" class="mb-4">
                    <h4 class="text-sm font-medium text-neutral-700 mb-2">New Media</h4>
...
const mediaPreviews = ref([]); // For new uploads
const existingMediaPreviews = ref([]); // For existing media
const error = ref('');
...
const fetchUpdate = async () => {
  try {
    ...
    // Load existing media files if they exist
    if (data.media_files && Array.isArray(data.media_files)) {
      existingMediaPreviews.value = [...data.media_files];
    }
  } catch (err) {
...
const handleMediaUpload = async (event) => {
  ...
  const totalFiles = existingMediaPreviews.value.length + mediaPreviews.value.length + files.length;
  if (totalFiles > 10) {
...
const removeExistingMediaItem = (index) => {
  existingMediaPreviews.value.splice(index, 1);
};
...
const saveUpdate = async () => {
  ...
  try {
    ...
    const newMediaFiles = mediaPreviews.value
    ...
    const allMediaFiles = [...existingMediaPreviews.value, ...newMediaFiles];
